...
const TableOrdersItems = ({ usrOrderItems, err }) => {
  ...
  const tableItemsView = usrOrderItems.map((item, index) => {
    ...
    if (item.order_status === "1") {
      badgeView = <span className="ps-badge black-badge">Pending</span>;
    } else if (item.order_status === "2") {
      badgeView = <span className="ps-badge purple-badge">Processing</span>;
    } else if (item.order_status === "3") {
      badgeView = <span className="ps-badge skyblue-badge">Shipping</span>;
    } else if (item.order_status === "4") {
      badgeView = <span className="ps-badge green-badge">Delivered</span>;
    } else if (item.order_status === "5") {
      badgeView = <span className="ps-badge yellow-badge">Returned</span>;
    } else if (item.order_status === "6") {
      badgeView = <span className="ps-badge red-badge">Cancelled</span>;
    } else if (item.order_status === "7") {
      badgeView = <span className="ps-badge orange-badge">Partial Delivery</span>;
    } else if (item.order_status === "18") {
      badgeView = <span className="ps-badge indigo-badge">Refunded</span>;
    } else if (item.order_status === "19") {
      badgeView = <span className="ps-badge greenyellow-badge">Picked</span>;
    } else if (item.order_status === "20") {
      badgeView = <span className="ps-badge blueviolet-badge">Confirmed</span>;
    }
